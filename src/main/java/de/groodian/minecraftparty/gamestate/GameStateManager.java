...
public class GameStateManager {
    ...
    private final Map<GameStates, GameState> gameStates;
    private final List<GameStates> gameStatsUsed;
    ...
    public String getCurrentGameStateName() {
        if (getCurrentGameState() instanceof LobbyState) {
            ...
            return "Ending";
        } else if (getCurrentGameState() instanceof MiniGame miniGame) {
            return PlainTextComponentSerializer.plainText().serialize(Messages.get(miniGame.name + ".name"));
        } else {
            return "Error";
        }
    }
