			...
			public void run() {
				for (Player player : plugin.getPlayers()) {
					if (player.getLocation().getX() != locations.getStart().getX() || player.getLocation().getZ() != locations.getStart().getZ()) {
						player.teleport(new Location(locations.getStart().getWorld(), locations.getStart().getX(), player.getLocation().getY(), locations.getStart().getZ(), player.getLocation().getYaw(),
								player.getLocation().getPitch()));
					}
				}
			}

		}.runTaskTimerAsynchronously(plugin, 4, 4);
			...
			public void run() {
				...
				for (Player player : plugin.getPlayers()) {
					...
					if (player.getLocation().distance(locations.getWin()) < 1 && checkpointsReached.get(player) == locations.getCheckpoints().size()) {
						checkpointsReached.put(player, 5);
						addWinner(player);
					}
